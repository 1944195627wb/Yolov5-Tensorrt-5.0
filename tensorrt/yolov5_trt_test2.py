...
class YoLov5TRT(object):
    ...
    def infer(self, raw_image_generator):
        ...
        for j in range(len(result_boxes)):
            ...
            result_names.append(self.categories[int(result_classid[j])])
        return batch_image_raw, end - start,result_names
    def destroy(self):
        # Remove any context from the top of the context stack, deactivating it.
        self.ctx.pop()
    ...
    def non_max_suppression(self, prediction, origin_h, origin_w, conf_thres=0.4, nms_thres=0.4):
        """
...
class inferThread(threading.Thread):
    ...
    def infer(self, frame):
        batch_image_raw, use_time,result_classid = self.yolov5_wrapper.infer(frame)
        print('time->{:.2f}ms'.format( use_time * 1000))
        return batch_image_raw, use_time,result_classid
...
class yolov5_tensorrtx(object):
    ...
    def detect(self,frame):
        img, t ,result_classid= self.thread1.infer(frame)
        return img, t ,result_classid
...
if __name__ == "__main__":
    ...
    yolov5=yolov5_tensorrtx(ROOT / "build2/libmyplugins.so",ROOT / "build2/yolov5s.engine",categories)
    ...
    try:
        if cap.isOpened():
            ...
            while cv2.getWindowProperty("show", 0) >= 0:
                _, frame = cap.read()
                img, t ,result_classid=yolov5.detect(frame)
                cv2.imshow("show",img)
